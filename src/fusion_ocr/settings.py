...
SETTINGS: tuple[Setting, ...] = (
    ...
    Setting("det_model", "str", settable=True,
            help="PaddleOCR detection model-name override ('' = engine default) — engine A/Bs"),
    Setting("rec_model", "str", settable=True,
            help="PaddleOCR recognition model-name override ('' = per-script default) — engine A/Bs"),
    ...
    Setting("move_processed", "bool", settable=True,
            help="watcher moves handled files to in/processed | in/failed (loop only)"),
    Setting("max_upload_mb", "float", settable=True, lo=1.0,
            help="API: reject an upload larger than this (MB) with 413"),
