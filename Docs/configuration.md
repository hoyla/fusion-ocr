...
| `apple_vision_skip_vlm` | `0.0` | `0.0`–`1.0` | When a page's mean Apple Vision confidence is ≥ this, skip the VLM read and let Vision's text stand in for the reading. **`0.0` disables (the default since 2026-09-16)**: priced by `eval_out/vision_skip_cost.py` — the old `0.92` default fired on 95% of pages and cost ~7 recall points and 5–6 placement points on clean scanned print, because Vision's confidence doesn't find the pages it misreads (pages rated ≥ 0.98 still lose 6). Same verdict as `paddle_skip_vlm`; enable only knowingly, on that evidence. |
| `paddle_skip_vlm` | `0.0` | `0.0`–`1.0` | The same cheap tier keyed on mean **PaddleOCR** confidence — the only VLM-skip available off macOS (Linux/VPC). `0.0` disables (the default): enable only on the evidence of `eval_out/paddle_skip_cost.py` (the Vision tier was benchmarked before enabling; this one must be too). |
| `det_model` | `""` | str | PaddleOCR **detection** model-name override (`""` = the engine default — which under the paddleocr 3.7 pin is `PP-OCRv6_medium_det`). The engine-generation A/B knob (`eval_out/engine_ab.py`). Output-affecting → fingerprinted. |
| `rec_model` | `""` | str | PaddleOCR **recognition** model-name override. Beware: an explicit value overrides the per-script recogniser selection, so leave `""` on multilingual estates. Output-affecting → fingerprinted. |
...
| Setting | Default | Configurable | What it does |
| --- | --- | --- | --- |
| `vlm.model` | `mlx-community/Qwen3.6-35B-A3B-4bit` | str | Default generalist reader model name passed to the OpenAI-compatible endpoint (rollback: `Qwen3.5-9B-MLX-4bit` — see stream F). |
